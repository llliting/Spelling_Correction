...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in range(1, len(sentence) - 1): #ignore <s> and </s>
      ...
      word = sentence[i]
      for (possibleword,possibility) in self.editModel.editProbabilities(word):
        trialSentence = sentence[:]
        trialSentence[i] = possibleword
        currScore = possibility + self.languageModel.score(trialSentence)
